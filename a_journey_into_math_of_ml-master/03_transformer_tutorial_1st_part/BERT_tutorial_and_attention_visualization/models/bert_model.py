...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        epsilon = 1e-8
        self.word_embeddings.weight.data = self.word_embeddings.weight.data.div(torch.norm(self.word_embeddings.weight, p=2, dim=1, keepdim=True).data+epsilon)
        self.token_type_embeddings.weight.data = self.token_type_embeddings.weight.data.div(torch.norm(self.token_type_embeddings.weight, p=2, dim=1, keepdim=True).data+epsilon)
    ...
    def forward(self, input_ids, positional_enc, token_type_ids=None,
                ner_probs=None, ensemble_mode=False, num_tags=None):
        """
        :param input_ids:
        :param positional_enc:
        :param token_type_ids:
        :param ner_probs: 是没有经过argmax的维特比输出
        :param nre_mode: 训练nre模型的时候设为True
        :return:
        """
        ...
        if ensemble_mode:
            token_type_embeddings = self.token_type_embeddings.weight[2: 2 + num_tags]
            # [num_tags, embed_dim]
            # ner_probs [batch, seq_len, num_tags]
            token_type_embeddings = torch.matmul(ner_probs, token_type_embeddings)
        else:
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, positional_enc, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True,
                get_attention_mask=False, ner_probs=None, ensemble_mode=False, num_tags=None, get_attention_matrices=False):
        ...
        embedding_output = self.embeddings(input_ids, positional_enc, token_type_ids,
                                           ner_probs=ner_probs, ensemble_mode=ensemble_mode, num_tags=num_tags)
        ...
        if not get_attention_mask:
            return encoded_layers, pooled_output
        else:
            return encoded_layers, pooled_output, attention_mask, extended_attention_mask
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, positional_enc, token_type_ids=None, attention_mask=None,
        ...
        # mlm_loss = self.mlm_loss_func(mlm_preds.transpose(1, 2), masked_lm_labels)
        # next_sen_loss = self.next_loss_func(next_sen_preds, next_sentence_label)
        # total_loss = mlm_loss + next_sen_loss
        # return total_loss, mlm_loss, next_sen_loss, mlm_preds, next_sen_preds
